...
# 1. Organograma como segunda aba.
s = s.replace(
...
# 2. Admissões recentes: grade com posições fixas para foto, nome e data.
old = '''        cards=[]
        for c in recentes:
...
# 3. Organograma: a tela principal mostra apenas o organograma; a hierarquia fica no ajuste recolhido.
    ...
    block = '''    with tab_organograma:
        ...
        if people:
            ...
            with st.expander("⚙ AJUSTAR ORGANOGRAMA",expanded=False):
                ...
                root_labels=list(root_opts.keys())
                root_index=list(root_opts.values()).index(saved_root) if saved_root in root_opts.values() else 0
                ...
                draft_root=str(root_opts[root_label])
                st.caption("Defina o superior imediato de cada colaborador. A visualização principal mostra somente o organograma.")
                novas_pais={}
                for c in people:
...
# 4. Colaboradores: cinco cartões por linha, com o cartão inteiro sendo o botão de seleção.
                ...
                for col,c in zip(cols,filtrados[base:base+5]):
                    ...
                    with col:
                        if c.get("foto_base64"):
                            ...
                            css="<style>div.st-key-card_colab_"+cid+" button{background-image:url('data:"+mime+";base64,"+b64+")!important;}</style>"
...
# 5. CSS de alinhamento e foco visual.
