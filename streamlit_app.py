...
if "cor_principal" not in st.session_state:
    st.session_state.cor_principal = "Amarelo (Padrão)"
if "estilo_botoes" not in st.session_state:
    st.session_state.estilo_botoes = "Amarelo"
...
PRIMARY = CORES[st.session_state.cor_principal]
IS_LIGHT = st.session_state.tema == "Claro"
...
/* CONFIGURAÇÕES */
.settings-title{{font-size:30px;font-weight:900;color:{TEXT};margin-bottom:4px}}
.settings-subtitle{{color:{MUTED};font-size:15px;margin-bottom:20px}}
...
.color-choice{{border:1px solid {BORDER};border-radius:12px;padding:16px;background:{INPUT_BG};height:100%}}
.color-dot{{width:26px;height:26px;border-radius:50%;display:inline-block;border:2px solid #ffffff55}}
</style>
