...
# ============================================================
# BANCO DE CONFIGURAÇÕES PERSISTENTES
# ============================================================
DEFAULT_CONFIG = {
    "tema": "Escuro (Padrão)",
...
def carregar_configuracoes():
    """Carrega a configuração global do Supabase sem cache."""
    try:
        resultado = (
            get_client()
            .table("almox_app_state")
            .select("estado")
            .eq("id", "global")
            .limit(1)
            .execute()
...
def salvar_configuracoes(config):
    """Persiste a configuração global no Supabase usando upsert."""
    payload = {
        "id": "global",
        "estado": config,
    }
...
# Carrega do banco uma vez por sessão. O botão de salvar atualiza e recarrega.
if "config_carregada" not in st.session_state:
    ...
    # Compatibilidade com sessões antigas que tenham somente as chaves individuais.
