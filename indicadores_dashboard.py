...
def _chart(rows, title):
    ...
    valores = [float(r.get("valor") or 0) for r in rows]
    metas = [float(r.get("meta") or 0) for r in rows if r.get("meta") is not None]
    maxv = max([100.0, *valores, *metas])
    n = len(rows)
    ...
    for r in rows:
        ...
        bars.append(f'''<div class="ind-bar-item">
          <div class="ind-bar-value" style="bottom:calc({h:.2f}% + 2px)">{_pct(valor)}</div>
...
def _indicator_card(name, rows, index):
    ...
    valor = float(latest.get("valor") or 0) if latest else None
    ...
    if latest is None:
        ...
        arrow = "↑" if status_cls == "ok" else "↓"

    title_num = f"{index:02d}"
    cards = f'''<div class="ind-kpis">
      <div class="ind-kpi"><div class="ind-kpi-label">ÚLTIMO RESULTADO</div><div class="ind-kpi-value">{_pct(valor) if valor is not None else '—'}</div><div class="ind-kpi-sub">Último lançamento</div></div>
      ...
      <div class="ind-kpi ind-status {status_cls}"><div class="ind-status-arrow">{arrow}</div><div><div class="ind-kpi-label">STATUS</div><div class="ind-status-text">{status}</div><div class="ind-kpi-sub">{len(rows)} lançamento(s) histórico(s)</div></div></div>
    </div>'''
...
def render_indicadores(indicadores):
    ...
    .ind-status.neutral:after{background:rgba(148,163,184,.07)}
    ...
    .ind-status.neutral .ind-status-arrow{background:rgba(148,163,184,.12);color:#a8b0ac}
    .ind-status-text{font-size:14px;font-weight:900;color:#4ade80;margin-top:4px}.ind-status.bad .ind-status-text{color:#ff6668}.ind-status.neutral .ind-status-text{color:#a8b0ac}
