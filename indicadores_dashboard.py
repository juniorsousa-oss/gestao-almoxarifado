...
def _chart(rows, title):
    ...
    maxv = max(100.0, max(float(r.get("valor") or 0), max(float(r.get("meta") or 0) for r in rows)) if rows else 100.0)
    n = len(rows)
    # Width is intentionally content-based so long histories remain readable inside a local scroll area.
    bars=[]
    for i,r in enumerate(rows):
        valor=float(r.get("valor") or 0)
        meta=float(r.get("meta") or 0)
        h=max(3,min(100,valor/maxv*100))
        label=pd.to_datetime(r.get("competencia"),errors="coerce")
        date_txt=label.strftime("%d/%m") if not pd.isna(label) else _safe(r.get("competencia") or "—")
        meta_pos=max(2,min(98,meta/maxv*100))
        bars.append(f'''<div class="ind-bar-item">
          <div class="ind-bar-value">{_pct(valor)}</div>
...
def _indicator_card(name, rows, index):
    ordered=sorted(rows,key=lambda r:str(r.get("competencia") or ""))
    latest=ordered[-1] if ordered else {}
    valor=float(latest.get("valor") or 0) if latest else 0
    meta=float(latest.get("meta") or 0) if latest and latest.get("meta") is not None else None
    diff=valor-meta if meta is not None else None
    status="Acima da meta" if diff is not None and diff>=0 else "Abaixo da meta"
    status_cls="ok" if diff is not None and diff>=0 else "bad"
    title_num=f"{index:02d}"
    cards=f'''<div class="ind-kpis">
      <div class="ind-kpi"><div class="ind-kpi-label">ÚLTIMO RESULTADO</div><div class="ind-kpi-value">{_pct(valor)}</div><div class="ind-kpi-sub">Último lançamento</div></div>
      ...
      <div class="ind-kpi"><div class="ind-kpi-label">DIFERENÇA</div><div class="ind-kpi-value">{_pp(diff) if diff is not None else '—'}</div><div class="ind-kpi-sub">Resultado − meta</div></div>
      <div class="ind-kpi ind-status {status_cls}"><div class="ind-status-arrow">{'↑' if status_cls=='ok' else '↓'}</div><div><div class="ind-kpi-label">STATUS</div><div class="ind-status-text">{status}</div><div class="ind-kpi-sub">{len(rows)} lançamento(s) histórico(s)</div></div></div>
    </div>'''
    return f'''<section class="ind-section">
      <div class="ind-section-title">{title_num} · {_safe(name)}</div>
      {cards}
      <div class="ind-chart-panel">
        <div class="ind-chart-head"><div><div class="ind-chart-title">Comparativo histórico</div><div class="ind-legend"><span><i class="ind-dot result"></i>Resultado</span><span><i class="ind-dot target"></i>Meta</span></div></div><div class="ind-view-buttons"><span class="active">TOTAL DE LANÇAMENTOS</span><span>AGRUPADO MÊS A MÊS</span></div></div>
        {_chart(ordered, name)}
      </div>
    </section>'''
...
def render_indicadores(indicadores):
    ...
    aliases={"ACURACIDADE DE ESTOQUE":"ACURÁCIA DE ESTOQUE","ACURÁCIA DE ESTOQUE":"ACURÁCIA DE ESTOQUE","ENTREGAS NO PRAZO":"ENTREGAS NO PRAZO","5S":"5S"}
    ...
    .ind-status.bad:after{background:rgba(255,77,79,.08)}
    ...
    .ind-status.bad .ind-status-arrow{background:rgba(255,77,79,.12);color:#ff6668}
    .ind-status-text{font-size:14px;font-weight:900;color:#4ade80;margin-top:4px}.ind-status.bad .ind-status-text{color:#ff6668}
    ...
    .ind-view-buttons{display:flex;gap:0;white-space:nowrap}.ind-view-buttons span{padding:9px 12px;border:1px solid #35413b;font-size:9px;font-weight:900;color:#aab2ae}.ind-view-buttons span:first-child{border-radius:9px 0 0 9px}.ind-view-buttons span:last-child{border-radius:0 9px 9px 0}.ind-view-buttons .active{background:#ffd43d;color:#111;border-color:#ffd43d}
    ...
    @media(max-width:900px){.ind-kpis{grid-template-columns:repeat(2,minmax(0,1fr))}.ind-chart-head{flex-direction:column}.ind-view-buttons{align-self:flex-end}}
    ...
    for i,(name,rows) in enumerate(groups.items(),1):
        st.markdown(_indicator_card(name,rows,i),unsafe_allow_html=True)
